...
На тестовой выборке из 1625 строк: **accuracy = 1.0, ROC-AUC = 1.0,
F1 = 1.0**, ошибок нет. Код, полная сохранённая модель, отчёты,
выполненный ноутбук и обязательные скриншоты находятся в ветке `lab1`.

[Проверка GitHub Actions](https://github.com/Vaiforo/pmrd2/actions/runs/34734969839)
завершилась успешно: 11 тестов, Ruff, проверка зависимостей, обучение,
независимое воспроизведение без данных и кэшей, выполнение ноутбука.
Все 8 проверяемых файлов и 3 DVC-указателя совпали побайтно между
двумя независимыми запусками в зафиксированном окружении.

- [Сохранённая модель](models/mushroom.joblib)
- [Выполненный ноутбук](notebooks/lab1_review.ipynb)
- [Тестовые метрики](reports/metrics.json)
- [Подтверждение воспроизводимости](reports/reproducibility.json)

Workflow повторяет проверки при изменении кода в `lab1` и сохраняет
полученные артефакты в эту же ветку. Его также можно запустить вручную.
...
отрисовки имитаций терминала. Все три изображения получены в успешном проверочном запуске.
